...
exports.vote_post = async function(req, res, next) {
    ...
    const post_id = req.body.post_id
    const user_id = req.user.id
    const res_posts = await pool.query(`SELECT p.post_votes, v.* FROM posts AS p 
                                        LEFT JOIN votedposts AS v 
                                            ON p.id = v.post_id AND v.user_id = $1 
                                        WHERE p.id = $2`, [user_id, post_id])
    const votes = res_posts.rows[0].post_votes
    ...
    if (res_posts.rows[0].id !== null) {
        if (res_posts.rows[0].liked === islike) {
            ...
            if (islike === true) {
                pool.query("UPDATE posts SET post_votes = $1 WHERE id = $2", [votes - 1, post_id])
            } else {
                pool.query("UPDATE posts SET post_votes = $1 WHERE id = $2", [votes + 1, post_id])
            }
        } else {
            ...
            if (islike === true) {
                pool.query("UPDATE posts SET post_votes = $1 WHERE id = $2", [votes + 2, post_id])
            } else {
                pool.query("UPDATE posts SET post_votes = $1 WHERE id = $2", [votes - 2, post_id])
            }
        }
